...
- If `$1` is empty: Execute **Basic Mode** (Steps 0-3). The agent MUST NOT suggest or create skills.
- If `$1` = `full`: Execute **Full Mode** (Steps 0-4).
- If `$1` = `user`: Switch directly to `user-onboarding-sop` and STOP.
  </objective>
      ...
      "options": [
        {
          ...
          "description": "Generate fresh AGENTS.md for this repo"
        },
        {
          "label": "Enhance existing",
          "description": "Improve current AGENTS.md without major rewrites"
        },
        {
          "label": "Replace",
          "description": "Start from scratch, ignore existing content"
        }
      ]
    },
    {
      ...
      "options": [
        { "label": "Full coverage", "description": "Document entire repo" },
        {
          ...
          "description": "Focus on dev workflow"
        },
...
1. Quickly gauge the codebase:
   - **MUST NOT** re-run `ls` or `tree` commands; use the injected listing at the top of this prompt.
...
- This is the middle ground between freedom and preservation.
- **Allowed**:
...
2. **Access Constraints**: Identify paths/files that are **read-only** or **restricted** for AI modification.
...
**Skip this step if `$1` is NOT `full`.**
After AGENTS.md is complete:

1. Identify repetitive tasks that would benefit from skills.
